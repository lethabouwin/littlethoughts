...
//   note: user_from_id is as in the posts table
    ...
    if (posts.length) {
      ...
      ${posts
        .map(post => {
          ...
          <img class="avatar-tiny" src="${post.user_from_id.avatar}"> <strong>${post.title}</strong>
        <span class="text-muted small">by ${post.user_from_id.username} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
      </a>`
        })
        .join("")}
    </div>`)
